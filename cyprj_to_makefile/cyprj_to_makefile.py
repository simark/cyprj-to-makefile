    ...
    def __call__(self, node):
        ...
        if node.tag == 'build_action':
            ...
            if self.last_build_action in self.interesting_build_actions:
                # Little hack... I don't know how to exclude C_FILEs from components.
                if '/API/' not in self.last_source_file:
                    self.source_files[self.last_build_action].append(self.last_source_file)
        elif node.tag == 'name_val_pair':
            name = node.attrib['name']
            value = node.attrib['v']
            
            if 'Additional Include Directories' in name:
                self.include_dirs += [fix_slashes(x.strip()) for x in value.split(';') if len(x) > 0]
...
def main():
    # Parse args
    argparser = argparse.ArgumentParser(description='Generate Makefile from Cypress cyprj.')
    argparser.add_argument('project_name', metavar='project-name', help='The project name.')
    argparser.add_argument('cyprj', help='The cyprj file.')
    argparser.add_argument('--cross', help='Cross compiler prefix (default: arm-none-eabi).', default='arm-none-eabi')
    argparser.add_argument('--objdir', help='The output directory (default: ./CortexM3/ARM_GCC_493/Debug).', default='./CortexM3/ARM_GCC_493/Debug')
    ...
    # Output the Makefile
    print(template.render(projname=args.project_name, cross=args.cross, objdir=args.objdir, visitor=visitor))
